...
<main class="ftdms-layout-content">
	 ...
	 <ul class="nav nav-tabs page-tabs">             
	    <li class="active"> <a href="#">管理活动打卡</a> </li>           
	 </ul>
...
<table style="width: 100%;text-align: center">
    <tr>
        <td>
             <form action="<%=path %>/recordList.action" method="post">
...
<strong> 活动名称:</strong><input name="cname" type="text" id="cname" style="width: 150px; "  />
    ...
    <table style="width:100%;text-align: center" id="show_tab">
                  ...
                  <tr>
                  <th >用户</th><th >活动名称</th><th >打卡内容</th>
                  <th >打卡时间</th>
                  <th scope="col">操作</th>
                  </tr>
                 <c:forEach items="${recordList}" var="record" varStatus="status" >
                     <tr>
                      <td >${record.uname}</td><td >${record.cname}</td><td >${record.cont}</td>
                      <td >${record.addtime}</td>
                      <td >
                      <a href="<%=path %>/torecordView.action?id=${record.id}" >详细</a>&nbsp;
<a href="javascript:void(0)"  onclick="layer.confirm('您确定要删除吗？',{icon:3,title:'提示'},function(index){
            layer.close(index);
            window.location.href='<%=path %>/recordDel.action?id=${record.id}';
        });">删除</a>&nbsp;
